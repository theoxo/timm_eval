...
from timm_eval.api.group import ConfigurableGroup
from timm_eval.api.metrics import (
    aggregate_subtask_metrics,
    pooled_sample_stderr,
    stderr_for_metric,
)
from timm_eval.api.task import Task
from timm_eval.utils import eval_logger, positional_deprecated
...
class TaskOutput:
    ...
    def calculate_aggregate_metric(self, bootstrap_iters=100000) -> None:
        for metric, items in self.sample_metrics.items():
            ...
            if isinstance(bootstrap_iters, int):
                ...
                self.agg_metrics[f"{metric}_stderr"] = (
...
def consolidate_results(
    ...
    @return: A tuple containing the consolidated results, samples, configs, and versions.
    ...
    The method then returns the consolidated results, samples, configs, and versions as a tuple.
    """
    ...
    for task_output in eval_tasks:
        ...
        for metric, items in task_output.sample_metrics.items():
            metric_key = str(metric)
            results[task_output.task_name][metric_key] = task_output.agg_metrics[
                metric_key
            ]
            results[task_output.task_name]["samples"] = task_output.sample_len
            results[task_output.task_name][f"{metric}_stderr"] = (
                task_output.agg_metrics[f"{metric}_stderr"]
            )
    return results, configs, versions, higher_is_better
