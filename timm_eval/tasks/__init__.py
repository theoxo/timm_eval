...
from timm_eval.api.group import ConfigurableGroup, GroupConfig
from timm_eval.api.task import ConfigurableTask, Task
from timm_eval.evaluator_utils import get_subtask_list
...
class TaskManager:
    """TaskManager indexes all tasks from the default `timm_eval/tasks/`
    ...
    def initialize_tasks(
        ...
        :param include_defaults: bool = True
            If set to false, default tasks (those in timm_eval/tasks/) are not indexed.
        :return
    ...
    def list_all_tasks(
        ...
        def sanitize_path(path):
            # don't print full path if we are within the timm_eval/tasks dir !
            # if we aren't though, provide the full path.
            if "timm_eval/tasks/" in path:
                return "timm_eval/tasks/" + path.split("timm_eval/tasks/")[-1]
            else:
                return path
                        ...
                        for attr in ["tag", "group"]:
                            if attr in config:
                                if attr == "group" and print_info:
                                    self.logger.info(
                                        "`group` and `group_alias` keys in TaskConfigs are deprecated and will be removed in v0.4.5 of timm_eval. "
                                        ...
                                        "Please see the v0.4.4 patch notes and our documentation: https://github.com/EleutherAI/timm-evaluation-harness/blob/main/docs/new_task_guide.md#advanced-group-configs "
...
def _check_duplicates(task_dict: dict) -> List[str]:
    ...
    Takes the output of timm_eval.evaluator_utils.get_subtask_list and
...
def get_task_dict(
    ...
    :param task_name_list: List[Union[str, Dict, Task]]
        Name of model or LM object, see timm_eval.models.get_model
